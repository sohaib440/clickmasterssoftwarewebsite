...
import { australiaLocation } from "@/data/locations";
...
export const metadata: Metadata = {
  title: pageTitle(australiaLocation.metaTitle ?? australiaLocation.title),
  description: australiaLocation.metaDescription ?? australiaLocation.description,
  ...selfCanonical("/location/software-house-and-software-development-company-in-australia"),
  openGraph: {
    title: pageTitleString(australiaLocation.metaTitle ?? australiaLocation.title),
    description: australiaLocation.metaDescription ?? australiaLocation.description,
    type: "website",
    locale: "en_AU",
  },
...
export default function AustraliaLocationPage() {
  const location = australiaLocation;
  const { sections } = location;

  const schemas = jsonLdGraph([
    organizationSchema,
    locationLocalBusinessSchema({
      areaServedName: "Australia",
    ...
    breadcrumbSchema([
      ...
      { name: "Australia", path: location.href },
    ]),
    ...
    <div className="flex min-h-full w-full flex-col overflow-x-clip bg-black text-foreground">
      ...
      <main className="flex w-full flex-1 flex-col overflow-x-clip">
        ...
        <SubLocationsSection
          ...
          title={
            <>
              Regions we serve across <span className="italic text-primary/95">Australia</span>
            </>
          }
        />
